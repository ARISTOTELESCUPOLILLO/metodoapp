    ...
    handlers: {
      POST: async ({ request }) => {
        try {
          ...
          if (!existing) {
            return Response.json({ code: 'not_found', message: 'Nenhuma voz para confirmar.' }, { status: 404 });
          }
          ...
          const { data: updated, error: updErr } = await supabaseAdmin
            ...
            .single();
          if (updErr) {
            return Response.json({ code: 'save_failed', message: 'Falha ao salvar.' }, { status: 500 });
          }
