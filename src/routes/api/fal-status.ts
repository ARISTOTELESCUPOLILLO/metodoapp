      ...
      GET: async ({ request }) => {
        try {
          ...
          if (!statusUrl || !responseUrl) {
            return Response.json({ error: 'statusUrl e responseUrl obrigatórios' }, { status: 400 });
          }
          ...
          if (!pollRes.ok) {
            ...
            return Response.json({ status: 'failed', error: `Erro ao verificar status: ${pollText.slice(0, 200)}` });
          }

          let pj: { status?: string };
          try { pj = JSON.parse(pollText); } catch { pj = {}; }

          const jobStatus = pj.status;
          console.log('[fal-status] status=%s', jobStatus);

          if (jobStatus === 'FAILED' || jobStatus === 'ERROR') {
            return Response.json({ status: 'failed', error: 'Geração de vídeo falhou na fila do fal.ai.' });
          }
          ...
          if (!finalRes.ok) {
            ...
            return Response.json({ status: 'failed', error: `Erro ao buscar resultado: ${finalText.slice(0, 200)}` });
          }
          ...
          if (!videoUrl) {
            ...
            return Response.json({ status: 'failed', error: 'Geração concluída mas sem URL de vídeo. Tente novamente.' });
          }
