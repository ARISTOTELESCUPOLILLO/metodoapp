...
async function falRun<T = unknown>(
  ...
  if (!submitRes.ok)
    throw new Error(`fal submit ${modelPath} ${submitRes.status}: ${submitText.slice(0, 400)}`);
  const submit = JSON.parse(submitText) as {
    status_url: string;
    response_url: string;
    status?: string;
  };
  ...
  while (status !== "COMPLETED" && Date.now() < deadline) {
    ...
    if (status === "FAILED" || status === "ERROR")
      throw new Error(`fal ${modelPath} falhou: ${txt.slice(0, 400)}`);
  }
  if (status !== "COMPLETED")
    throw new Error(`fal ${modelPath} timeout (status=${status ?? "?"}).`);
        ...
        try {
          ...
          if (row.provider === "elevenlabs") {
            ...
            if (!ttsRes.ok) {
              ...
              return Response.json(
                { error: `ElevenLabs TTS ${ttsRes.status}: ${err.slice(0, 200)}` },
                { status: 502 },
              );
            }
            ...
            return Response.json(
              { error: "Voz Chatterbox não suportada. Reconfigure no Kit Imagem." },
              { status: 400 },
            );
          } else {
            // Legado MiniMax
          ...
          if (!audioUrl)
            return Response.json({ error: "fal não retornou áudio." }, { status: 502 });
