  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        if (!systemToken) return Response.json({ success: false, error: 'ACCESS_TOKEN não configurado' }, { status: 403 });
        ...
        if (!image_url || !caption) {
          return Response.json({ success: false, error: 'image_url e caption são obrigatórios' }, { status: 400 });
        }
        ...
        try {
          ...
          if (!pageTokenData.access_token) throw new Error(pageTokenData.error?.message || 'Falha ao obter Page Access Token');
