  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        if (!token) return Response.json({ success: false, error: 'ACCESS_TOKEN não configurado' }, { status: 403 });
        if (!igId)  return Response.json({ success: false, error: 'IG_BUSINESS_ID não configurado' }, { status: 403 });
