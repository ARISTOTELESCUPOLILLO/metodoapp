      ...
      GET: async ({ request }) => {
        ...
        try {
          ...
          const [pageRes, igRes] = await Promise.all([
            fetch(
              `https://graph.facebook.com/${META_VERSION}/${PAGE_ID}?fields=id,name&access_token=${devToken}`,
            ),
            fetch(
              `https://graph.facebook.com/${META_VERSION}/${IG_USER_ID}?fields=id,username&access_token=${devToken}`,
            ),
          ]);
