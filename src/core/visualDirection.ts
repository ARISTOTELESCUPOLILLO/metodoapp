...
- Quando aparecer notebook, tablet, celular, monitor ou TV, a TELA FRONTAL deve mostrar conteúdo real e coerente com o tema (gráfico, dashboard, app, mensagem, foto, planilha, vídeo em playback). Proibido: tela apagada, preta, branca, lockscreen, wallpaper de fábrica, logo de sistema operacional, blur sem conteúdo, placeholder. A imagePrompt DEVE descrever literalmente o que aparece na tela.
- TAMPA / VERSO / CARCAÇA — DEFEITO FÍSICO PROIBIDO (vale para notebook, tablet, celular, smartphone, monitor e desktop): a carcaça traseira, tampa e verso de QUALQUER dispositivo são superfícies físicas SÓLIDAS e OPACAS (plástico, alumínio, vidro, couro). NÃO têm tela. NÃO emitem luz. NÃO exibem conteúdo. PROIBIDO ABSOLUTAMENTE: imagem de área de trabalho, display, interface, app, dashboard, ícone ou qualquer conteúdo visual de tela LCD/OLED renderizado sobre a carcaça traseira, tampa ou verso de qualquer dispositivo; duplicação ou espelho da tela frontal na carcaça traseira; brilho ou emissão luminosa simulando display no verso; efeito "tela invertida"; aparência de monitor dos dois lados. A carcaça traseira PODE aparecer parcialmente na cena como superfície neutra — o que NUNCA pode existir é conteúdo visual de tela aplicado sobre ela. COMPORTAMENTO FÍSICO CORRETO: carcaça sólida e neutra, tela exclusivamente na face interna, dispositivo fisicamente coerente e realista.
- ENQUADRAMENTO OBRIGATÓRIO: câmera posicionada FRONTALMENTE À TELA — perfil/diagonal sobre a mesa, na mão em uso, no colo, câmera rente à superfície. A imagePrompt DEVE especificar "câmera frontal à tela" ou equivalente. PROIBIDO: câmera posicionada atrás do aparelho, top-down de laptop fechado, mockup plano mostrando a carcaça traseira como elemento principal.
