...
const VISUAL_DIRECTIONS: Record<MoodCode, VisualDirection> = {
  ...
  'OP-06': {
    ...
    camera: 'câmera 50mm ou 70mm, enquadramento médio próximo, altura dos olhos, distância acolhedora, composição limpa, sem grão, acabamento suave e silencioso',
...
const DESVIO_SYMBOLIC_RUPTURE_VARIATIONS: string[] = [
  'OBJETO DESLOCADO: objeto comum colocado em lugar inesperado dentro da cena — item de trabalho pousado em superfície incomum, peça cotidiana em posição estranha mas fisicamente apoiada no ambiente (sobre mesa, chão, parede, balcão). O objeto ESTÁ na cena, não voa nem flutua. Câmera OBRIGATORIAMENTE abaixo da cintura do personagem (contra-plongée visível, lente 28-35mm, linhas verticais convergindo) revelando o estranhamento do posicionamento — nunca na altura dos olhos. Personagem de um lado; objeto deslocado no outro extremo ou em primeiro plano. Não usar livro, megafone, notebook, porta, chave solta ou dashboard. AMBIENTE: espaço doméstico com elemento profissional infiltrado — sala de estar, cozinha residencial, varanda, quarto — onde o contraste entre o cenário pessoal e o objeto de trabalho É o estranhamento; nunca escritório corporativo.',
  'SOMBRA OU AUSÊNCIA: a cena mostra a SOMBRA ou o REFLEXO projetado de algo que não está presente no quadro — sombra caindo sobre superfície real (chão, parede, mesa), nunca flutuante. O personagem e o ambiente estão nítidos; apenas a sombra revela o elemento ausente. A ausência sugere o problema ou a oportunidade da peça. Câmera OBRIGATORIAMENTE angulada — contra-plongée acentuado ou lateral extremo rasante ao chão — com distorção de perspectiva claramente visível; composição diagonal que destaca a sombra. AMBIENTE: espaço industrial ou de produção — galpão, oficina, estoque, fábrica, área de carga — com luz dura lateral de fonte única projetando a sombra do elemento ausente sobre piso ou parede real.',
  'ESCALA ALTERADA: um elemento cotidiano aparece MAIOR ou MENOR do que o esperado — mas integrado ao ambiente, fisicamente presente na cena. Objeto desproporcional pousado (enorme sobre a mesa, minúsculo nas mãos do personagem, gigante apoiado ao fundo). Personagem interage ou está próximo revelando a desproporção por comparação. Câmera OBRIGATORIAMENTE em plongée acentuado (de cima) ou contra-plongée extremo (rente ao chão), lente 28-35mm, perspectiva visivelmente distorcida — nunca câmera neutra na altura dos olhos. AMBIENTE: espaço externo urbano — rua, calçada, fachada, pátio, marquise — onde a comparação de escala fica evidente à luz do dia, ao ar livre.',
  'COR INESPERADA: um único item cotidiano presente fisicamente na cena recebe cor incomum dentro da paleta do mood — objeto está na mão do personagem, sobre superfície ou integrado ao ambiente, não flutuante. Câmera OBRIGATORIAMENTE contra-plongée ou plongée com distorção de perspectiva visível — jamais frontal neutra; luz teatral lateral ou de baixo salienta o contraste cromático. Apenas esse elemento tem a cor conceitual; o restante da cena segue a paleta fria/escura do mood. AMBIENTE: galeria, vitrine, espaço expositivo ou fundo neutro de tom escuro controlado — onde o item de cor inesperada se isola visualmente sem concorrência de outros elementos.',
];
