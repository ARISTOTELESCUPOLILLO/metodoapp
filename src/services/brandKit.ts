...
export const loadKitServer = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: row } = await supabaseAdmin
      .from('brand_kits').select('*').eq('user_id', targetId).maybeSingle();
  ...
  .handler(async ({ data, context }) => {
    ...
    if (existing?.id) {
      const { data: d, error } = await supabaseAdmin
        .from('brand_kits').update(row).eq('id', existing.id).select('*').single();
      if (error) throw new Error(error.message);
      savedRow = d;
    } else {
      const { data: d, error } = await supabaseAdmin
        .from('brand_kits').insert(row).select('*').single();
      if (error) throw new Error(error.message);
      savedRow = d;
    }
