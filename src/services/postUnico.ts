...
const MOOD_INSTRUCTIONS: Record<MoodCode, string> = {
  ...
  'OP-02': 'Direção visual IMPACTO — gramática fixa: fundo escuro médio (preto/grafite), EXATAMENTE 1 cor quente saturada (amarelo, laranja ou vermelho) como acento dramático, luz focal direcional, contraste pronunciado, câmera em contra-plongée leve ou 3/4 dinâmico. Esta gramática é inegociável quando o mood for OP-02. Dentro dela, explore variação de execução para não repetir a fórmula mais óbvia: pode ser cinematográfico quente (luz âmbar/dourada focada), urbano de alto contraste (rua, neon, sombra dura, granulação) ou gráfico saturado (cor sólida chapada como fundo, sujeito recortado, estética pôster) — em todas as variações a cor quente saturada deve estar presente sobre fundo escuro. PROIBIDO: câmera frontal na altura dos olhos, câmera de cima para baixo. VÍCIOS A EVITAR: papel ou prancheta como prop automático, executivo de terno como padrão, olhar lateral repetido, fundo corporativo genérico, plantas e vasos decorativos. OPÇÃO SEM PERSONAGEM CENTRAL: quando produto, objeto ou elemento de marca for o protagonista mais forte, construir a cena sem figura humana dominante — foco no sujeito real com luz recortada e composição dramática. IMPACTO se aplica a qualquer segmento — ambiente e gesto pertencem ao negócio real.',
...
Hierarquia tipográfica: título dominante em CAIXA ALTA e texto de apoio com corpo no mínimo 65% do título (NÃO é legenda pequena — deve ser legível sem zoom no celular).`;
