...
const OBJETIVO_ARCHETYPES: Record<Exclude<PostUnicoObjetivo, 'nenhum'>, string[]> = {
  oportunidade: [
    ...
    'CONCEITO DESTA GERAÇÃO — GESTO DECISIVO: foco em close de mão em ação digital — prestes a pressionar tecla, deslizando dedo em tela de tablet, segurando smartphone ou caneta stylus, apontando para tela. A urgência está no gesto decisivo e digital, não no movimento do corpo inteiro. PROIBIDO: mão escrevendo em caderno de papel, caneta sobre papel físico.',
    ...
    'CONCEITO DESTA GERAÇÃO — OBJETO SIMBÓLICO: elemento físico concreto que representa decisão e momento único (notebook fechado ou tablet em pausa sobre mesa de trabalho, smartphone face down, chave sobre superfície, item característico do negócio digital). Composição limpa, sem pessoa como foco principal.',
