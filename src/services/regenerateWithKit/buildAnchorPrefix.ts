...
export function buildAnchorPrefix(
  ...
  if (refs.fachada) {
    ...
    const fachadaClause = `${fachadaAmbienteClause}. TEXTO/NOME DO ESTABELECIMENTO NA FACHADA: se o nome ou a marca do estabelecimento JÁ aparecer nítido e legível na foto de referência (pintado na parede, em placa, testeira, faixa ou letreiro), PRESERVE-O fielmente como está na foto — mesma grafia, mesma posição, mesmas cores — exatamente como você preserva a arquitetura e as cores do local; NÃO apague, NÃO borre e NÃO substitua esse nome por uma área lisa/branca/genérica. O que é PROIBIDO é INVENTAR ou redesenhar do zero um letreiro/marca que NÃO esteja legível na referência: onde não houver escrita clara, mantenha apenas a forma/o suporte (a placa, a testeira, a faixa) de modo neutro, SEM criar letras ou marca fictícia. A logomarca oficial é aplicada depois, fora da IA, em outra área da peça — ela NÃO substitui o nome que já existe na fachada real. Se houver fios elétricos, postes, cabos aéreos, lixo ou poluição visual cruzando a fachada, é PERMITIDO retocar/remover esses elementos da composição final — desde que a arquitetura, as cores e o nome do local continuem plenamente reconhecíveis. Se o céu aparecer na cena, é PERMITIDO substituí-lo por um céu mais bonito e coerente com o mood e o horário do dia (ex.: azul limpo, entardecer dourado, nublado suave) — sem look artificial ou composição com aparência de colagem`;
    lines.push(
      reiluminaFachada
  ...
  if (refs.produtos?.length) {
    ...
    const telaClause = refs.produtoTelaInformativa
      ? " A TELA deste produto exibe conteúdo que É a identidade do produto — fica em PRIMEIRO PLANO, exposto (não em uso), com a tela voltada para a câmera e reproduzida com nitidez total (ver ⚠ CONTENÇÃO ABSOLUTA DO CONTEÚDO DE TELA e PRODUTO EXPOSTO — NÃO EM USO no início do prompt)."
