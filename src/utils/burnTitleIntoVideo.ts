...
async function loadVideoBytes(url: string): Promise<Uint8Array> {
  try {
    ...
    if (!u || u.byteLength < 1024) {
      throw new Error(`Não foi possível baixar o vídeo: ${(e as Error)?.message || 'erro desconhecido'}`);
    }
    return u;
  }
}
...
export async function burnTitleIntoVideo(
  ...
  try {
    ...
    console.log('[burnTitle] input mp4:', videoBytes.byteLength, 'bytes; png:', pngBytes.byteLength, 'bytes');
    ...
    console.log('[burnTitle] output mp4:', bytes.byteLength, 'bytes em', Math.round(performance.now() - t0), 'ms');
