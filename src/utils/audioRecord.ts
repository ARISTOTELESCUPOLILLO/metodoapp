...
export async function validateAudioBlob(blob: Blob): Promise<AudioValidation> {
  if (blob.size > VOICE_MAX_BYTES) {
    return { ok: false, durationS: 0, rms: 0, reason: `Arquivo grande demais (>${Math.round(VOICE_MAX_BYTES / 1024 / 1024)} MB).` };
  }
  const arrBuf = await blob.arrayBuffer();
  const AC: typeof AudioContext = (window.AudioContext || (window as any).webkitAudioContext);
  if (!AC) return { ok: false, durationS: 0, rms: 0, reason: 'Navegador sem suporte a Web Audio API.' };
  const ctx = new AC();
  ...
  try {
    ...
    return { ok: false, durationS: 0, rms: 0, reason: 'Formato de áudio não suportado. Use mp3, m4a, wav ou webm.' };
  }
  ...
  let n = 0;
  for (let i = 0; i < ch.length; i += stride) { sumSq += ch[i] * ch[i]; n++; }
  const rms = Math.sqrt(sumSq / Math.max(1, n));
  try { await ctx.close(); } catch {}

  if (durationS < VOICE_MIN_SECONDS) {
    return { ok: false, durationS, rms, reason: `Áudio muito curto (${Math.round(durationS)}s). Mínimo de ${VOICE_MIN_SECONDS}s.` };
  }
  if (durationS > VOICE_MAX_SECONDS) {
    return { ok: false, durationS, rms, reason: `Áudio muito longo (${Math.round(durationS)}s). Máximo de ${VOICE_MAX_SECONDS}s.` };
  }
  if (rms < 0.005) {
    return { ok: false, durationS, rms, reason: 'Áudio com volume muito baixo ou silencioso. Grave mais próximo do microfone.' };
  }
...
export async function startRecorder(mimeHint?: string): Promise<RecorderHandle> {
  ...
  for (const m of candidates) {
    if ((window as any).MediaRecorder && (MediaRecorder as any).isTypeSupported?.(m)) { chosen = m; break; }
  }
  const rec = chosen
