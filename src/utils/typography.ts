...
const SECONDARY_FONT_SHAPES: Record<SecondaryFont, string> = {
  ...
  grossa:
    'cursiva conectada, traço de PINCEL GROSSO — linhas espessas e cheias do início ao fim de cada letra, sem afinar em curvas, hastes ou ligaduras, peso visual equivalente a um brush lettering BOLD (não o extremo EXTRA-BOLD/HEAVY), inclinação leve para a direita, floreios expressivos igualmente grossos — estilo "lettering a pincel bold", evite traço fino, hairline ou delicado em qualquer trecho',
};
...
export function buildScriptAccentBlock(style: SecondaryFont, titulo: string): string {
  ...
  const order = [
    role,
    ...(["fechamento", "assunto", "beneficio"] as AccentRole[]).filter((r) => r !== role),
  ];
