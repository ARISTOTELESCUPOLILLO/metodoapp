...
export function getGenStore(): GenStore { return _store; }
export function getGenStoreUserId(): string | null { return _storeUserId; }
  ...
  if (userId) {
    try {
      if ('result' in patch) {
        ...
        patch.result === undefined ? localStorage.removeItem(k) : localStorage.setItem(k, JSON.stringify(patch.result));
      }
      if ('postUnicoImg' in patch) {
        const k = `metodo-op-postunico-img-v1:${userId}`;
        patch.postUnicoImg === undefined ? localStorage.removeItem(k) : localStorage.setItem(k, JSON.stringify(patch.postUnicoImg));
      }
      if ('caption' in patch) {
        const k = `metodo-op-postunico-caption-v1:${userId}`;
        patch.caption === undefined ? localStorage.removeItem(k) : localStorage.setItem(k, JSON.stringify(patch.caption));
      }
      if ('postUnicoStarted' in patch) {
        localStorage.setItem(`metodo-op-postunico-started-v1:${userId}`, patch.postUnicoStarted ? 'true' : 'false');
      }
    } catch {}
  }
  _notify();
