  ...
  return (
    <section className="panel">
      <div
        ...
        style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 24, flexWrap: 'wrap', cursor: 'pointer', userSelect: 'none' }}
      >
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          {!isOpen && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 4, flexWrap: 'wrap' }}>
              {kit.companyName && (
                <span style={{ fontSize: 13, color: '#475569', fontWeight: 600 }}>{kit.companyName}</span>
              )}
              {(kit.primaryColor || kit.secondaryColor || kit.accentColor) && (
                <div style={{ display: 'flex', gap: 4 }}>
                  {[kit.primaryColor, kit.secondaryColor, kit.accentColor].map((c, i) =>
                    c ? <span key={i} style={{ width: 12, height: 12, borderRadius: '50%', background: c, border: '1.5px solid #e2e8f0', display: 'inline-block' }} /> : null
                  )}
                </div>
              )}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <svg
            width={18} height={18} viewBox="0 0 24 24" fill="none" stroke="#64748b" strokeWidth={2.5}
