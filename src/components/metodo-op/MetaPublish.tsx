...
export function MetaPublish({ imageDataUrl, caption }: Props) {
  const [igState, setIgState] = useState<'idle' | 'loading' | 'ok' | 'err'>('idle');
  const [fbState, setFbState] = useState<'idle' | 'loading' | 'ok' | 'err'>('idle');
  ...
  async function publishInstagram() {
    ...
    try {
      const res = await fetch('/api/meta/publish-instagram', {
        ...
        body: JSON.stringify({ imageDataUrl, caption: caption || '' }),
      });
      const d = await res.json() as { success?: boolean; error?: string };
      if (!d.success) throw new Error(d.error || 'Erro ao publicar');
      setIgState('ok');
    } catch (e) {
      setIgError((e as Error).message);
      setIgState('err');
    }
  }
  ...
  async function publishFacebook() {
    ...
    try {
      const res = await fetch('/api/meta/publish-facebook', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', ...(await authHeader()) },
        body: JSON.stringify({ imageDataUrl, text: caption || '' }),
      });
      const d = await res.json() as { success?: boolean; error?: string };
      if (!d.success) throw new Error(d.error || 'Erro ao publicar');
      setFbState('ok');
  ...
  return (
    <div style={{ display: 'grid', gap: 8 }}>
      ...
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
        {/* Instagram */}
        {igState === 'ok' ? (
          <div style={{ padding: '9px 16px', borderRadius: 10, background: '#f0fdf4', border: '1px solid #86efac', color: '#15803d', fontSize: 13, fontWeight: 700 }}>
          ...
          <button
            ...
            style={{ ...btnStyle(igState === 'loading'), background: 'linear-gradient(135deg,#833AB4,#C13584,#E1306C,#FD1D1D)', color: '#fff' }}
          >
            ...
            {igState === 'loading' ? 'Publicando…' : 'Publicar no Instagram'}
          </button>
        )}

        {/* Facebook */}
        {fbState === 'ok' ? (
          <div style={{ padding: '9px 16px', borderRadius: 10, background: '#eff6ff', border: '1px solid #93c5fd', color: '#1d4ed8', fontSize: 13, fontWeight: 700 }}>
          ...
          <button
            ...
            style={{ ...btnStyle(fbState === 'loading'), background: '#1877F2', color: '#fff' }}
          >
            <svg width="14" height="14" viewBox="0 0 24 24" fill="white"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
            {fbState === 'loading' ? 'Publicando…' : 'Publicar no Facebook'}
          </button>
        )}
      </div>
