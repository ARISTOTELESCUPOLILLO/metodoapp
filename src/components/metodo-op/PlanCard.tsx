...
export function PlanCard({ slot, isAdmin }: Props) {
  ...
  const imgsPct =
    slot.imgsLimiteDisplay > 0
      ? Math.min(99, Math.round((slot.imgsUsadas / slot.imgsLimiteDisplay) * 100))
      : 0;
  ...
  const renderPct =
    slot.rendersLimiteDisplay > 0
      ? Math.min(99, Math.round((slot.rendersUsados / slot.rendersLimiteDisplay) * 100))
      : 0;
  ...
  const expiryLabel =
    cycle.status === "none"
      ? "sem data"
      : cycle.status === "overdue"
        ? `vencido há ${Math.abs(cycle.daysLeft)}d`
        : cycle.daysLeft === 0
          ? "vence hoje"
    ...
    >
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
        <div>
          ...
          <div
            style={{ color: "rgba(255,255,255,.50)", fontSize: 10, marginTop: 1, lineHeight: 1.3 }}
          >
            {slot.plan.nome}
          </div>
        </div>
        ...
        <div style={{ textAlign: "right", flexShrink: 0, marginLeft: 8 }}>
          <div style={{ color: expiryColor, fontSize: 10, fontWeight: 700 }}>{expiryLabel}</div>
          {expiryDate && (
      ...
      {hasImgsSection && (
        <div>
          <div style={{ fontSize: 9, color: "rgba(255,255,255,.40)", marginBottom: 2 }}>
            imagens
          </div>
          {imgsExhausted ? (
            <div style={{ fontSize: 10, color: "#ef4444", fontWeight: 700, letterSpacing: 0.2 }}>
              🔒 Esgotado
            </div>
          ) : (
            hasImgsBar && (
