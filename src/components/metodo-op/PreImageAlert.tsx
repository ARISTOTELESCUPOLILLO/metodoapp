...
export default function PreImageAlert({ open, onConfirm, onCancel }: Props) {
  ...
  useEffect(() => {
    if (!open) { setDontShow(false); return; }
  ...
  return (
    ...
    >
      <div
        ...
        style={{ background: '#fff', borderRadius: 18, padding: 26, maxWidth: 460, width: '100%', boxShadow: '0 30px 80px rgba(15,23,42,.35)', border: '1px solid #e2e8f0' }}
      >
        <h3 style={{ margin: 0, fontSize: 19, fontWeight: 800, color: '#0f172a' }}>Antes de gerar a imagem</h3>
        <p style={{ margin: '10px 0 16px', color: '#475569', fontSize: 14.5, lineHeight: 1.55 }}>
          A IA vai usar o <strong>título</strong> e o <strong>texto</strong> deste bloco como base visual.
          Se quiser ajustar, refine agora — depois de gerar a imagem, mudar o texto não muda a peça.
        </p>

        <label style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 18, fontSize: 13, color: '#475569', cursor: 'pointer' }}>
        ...
        <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end', flexWrap: 'wrap' }}>
          <button
            ...
            type="button"
            onClick={onCancel}
            style={{ background: '#f8fafc', color: '#0f172a', border: '1px solid #cbd5e1', borderRadius: 12, padding: '10px 16px', fontWeight: 700, fontSize: 14, cursor: 'pointer' }}
          >
            Revisar texto
          </button>
          <button
            ...
            style={{ background: '#123a63', color: '#fff', border: 'none', borderRadius: 12, padding: '10px 18px', fontWeight: 700, fontSize: 14, cursor: 'pointer', boxShadow: '0 8px 20px rgba(18,58,99,.3)' }}
          >
            Gerar imagem
          </button>
        </div>
 ...
 */
function RegenAlert({ open, tipo, onConfirm, onCancel }: { open: boolean; tipo: RegenTipo; onConfirm: () => void; onCancel: () => void }) {
  const cancelRef = useRef<HTMLButtonElement>(null);
  ...
  return (
    ...
    >
      <div
        ...
        style={{ background: '#fff', borderRadius: 18, padding: 26, maxWidth: 460, width: '100%', boxShadow: '0 30px 80px rgba(15,23,42,.35)', border: '1px solid #e2e8f0' }}
      >
        <h3 style={{ margin: 0, fontSize: 19, fontWeight: 800, color: '#0f172a' }}>Gerar nov{tipo === 'Vídeo' ? 'o' : 'a'} {tipo}?</h3>
        <p style={{ margin: '10px 0 18px', color: '#475569', fontSize: 14.5, lineHeight: 1.55 }}>
          Este pedido vai consumir uma geração de <strong>{tipo}</strong> do limite disponível no seu plano.
        </p>
...
export function useImageGenAlert() {
  ...
  const [pendingRegen, setPendingRegen] = useState<null | { run: () => void; tipo: RegenTipo }>(null);
  ...
  function cancelRegen() { setPendingRegen(null); }

  const dialog = (
    <>
      <PreImageAlert open={!!pending} onConfirm={confirm} onCancel={cancel} />
      <RegenAlert open={!!pendingRegen} tipo={pendingRegen?.tipo || 'Estático'} onConfirm={confirmRegen} onCancel={cancelRegen} />
    </>
  );
