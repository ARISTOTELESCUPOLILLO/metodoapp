...
export function PlansTab() {
  ...
  async function save() {
    ...
    setMsg(null);
    const payload = {
      codigo: editing.codigo, nome: editing.nome, tipo: editing.tipo,
  ...
  async function remove(p: Plan) {
    const { count } = await supabase.from('profiles').select('id', { count: 'exact', head: true }).eq('plano_id', p.id);
    if (count && count > 0) { alert(`Não é possível excluir: ${count} usuário(s) usando este plano.`); return; }
    ...
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 12 }}>
        ...
        <button onClick={() => setEditing({ ...empty })} style={{
          background: '#0f213f', color: '#fff', border: 'none', padding: '8px 14px',
          ...
          {plans.map((p) => (
            <div key={p.id} style={mCard}>
              <div style={{ display: 'flex', justifyContent: 'space-between', gap: 8, marginBottom: 8 }}>
                ...
                <span style={{ fontSize: 11, fontWeight: 600, padding: '2px 8px', borderRadius: 999, background: p.ativo ? '#dcfce7' : '#fee2e2', color: p.ativo ? '#166534' : '#991b1b', height: 'fit-content' }}>
              ...
              <div style={{ display: 'flex', gap: 6, marginTop: 10 }}>
                <button onClick={() => setEditing(p)} style={{ ...btn, flex: 1 }}>Editar</button>
                <button onClick={() => remove(p)} style={{ ...btn, flex: 1, color: '#b91c1c' }}>Excluir</button>
              </div>
            </div>
          ))}
        </div>
            ...
            <tbody>
              {plans.map((p) => {
                ...
                <tr key={p.id} style={{ borderTop: '1px solid #e2e8f0' }}>
                  ...
                  <Td>
                    <button onClick={() => setEditing(p)} style={btn}>Editar</button>{' '}
      ...
      {editing && (
        <div style={overlay} onClick={() => setEditing(null)}>
          <div style={modal} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={{ display: 'grid', gap: 10 }}>
              <Inp label="Código" value={editing.codigo} onChange={(v) => setEditing({ ...editing, codigo: v })} />
              <Inp label="Nome" value={editing.nome} onChange={(v) => setEditing({ ...editing, nome: v })} />
              <label style={{ display: 'grid', gap: 4 }}>
                <span style={lbl}>Tipo</span>
                <select value={editing.tipo} onChange={(e) => setEditing({ ...editing, tipo: e.target.value })} style={inp}>
              ...
              <div style={{ marginTop: 6, padding: '10px 12px', background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 8 }}>
                ...
                <div style={{ fontSize: 11, color: '#64748b', marginBottom: 10 }}>
                  Quantidade que vem "de fábrica" neste plano. Soma com os outros planos do usuário + extras dele.
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                  <Inp label="Estático" type="number" value={String(editing.base_estatico ?? 0)} onChange={(v) => setEditing({ ...editing, base_estatico: Number(v) })} />
                  <Inp label="Carrossel" type="number" value={String(editing.base_carrossel ?? 0)} onChange={(v) => setEditing({ ...editing, base_carrossel: Number(v) })} />
                  <Inp label="Estático final" type="number" value={String(editing.base_estatico_final ?? 0)} onChange={(v) => setEditing({ ...editing, base_estatico_final: Number(v) })} />
                  <Inp label="Reels" type="number" value={String(editing.base_reels ?? 0)} onChange={(v) => setEditing({ ...editing, base_reels: Number(v) })} />
                </div>
              </div>
              ...
              <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', marginTop: 8 }}>
                ...
                <button onClick={save} disabled={saving} style={{
                  background: '#0f213f', color: '#fff', border: 'none', padding: '8px 14px',
                  borderRadius: 6, fontWeight: 600, cursor: 'pointer', fontSize: 13, opacity: saving ? .6 : 1,
...
const mCard: React.CSSProperties = { background: '#fff', border: '1px solid #e2e8f0', borderRadius: 10, padding: 14 };
