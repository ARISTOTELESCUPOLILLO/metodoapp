...
export function StorageTab() {
  ...
  const avgBytesPerUser = activeUsers.length > 0
    ? stats!.totalBytes / activeUsers.length
    : 0;

  const cabeAtual = avgBytesPerUser > 0
    ? Math.max(0, Math.floor(freeBytes / avgBytesPerUser))
    : null;
    ...
    <div style={{ maxWidth: 860 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20, flexWrap: 'wrap' }}>
        ...
        <button
          ...
          disabled={loading}
          style={{
            padding: '6px 14px', fontSize: 13, fontWeight: 600, cursor: loading ? 'default' : 'pointer',
            background: '#0f213f', color: '#fff', border: 'none', borderRadius: 6,
      ...
      {stats && (
        <>
          ...
          <div style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 10, padding: 20, marginBottom: 20 }}>
            <div style={{ fontWeight: 700, fontSize: 15, marginBottom: 12 }}>Previsão de capacidade</div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: 12 }}>
              <Metric label="Usuários reais com storage" value={String(activeUsers.length)} />
              <Metric label="Uso médio por usuário" value={avgBytesPerUser > 0 ? fmtMB(avgBytesPerUser) : '—'} />
              <Metric label="Máximo teórico/usuário" value="47 MB" sub="48 imgs + 6 renders" />
              {cabeAtual !== null && (
                <Metric label="Ainda cabem (ritmo atual)" value={`~${cabeAtual} usuários`} highlight />
              )}
              <Metric label="Ainda cabem (pior caso)" value={`~${cabePiorCaso} usuários`} highlight />
            </div>
          </div>
                  ...
                  <tbody>
                    {stats.byUser.map((u) => (
                      <tr key={u.user_id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                        ...
                        <td style={{ padding: '8px 10px', color: '#64748b' }}>{u.email ?? u.user_id.slice(0, 8) + '…'}</td>
                        <td style={{ padding: '8px 10px', fontVariantNumeric: 'tabular-nums' }}>{fmtMB(u.bytes)}</td>
                        <td style={{ padding: '8px 10px', color: '#64748b' }}>{u.files}</td>
                        <td style={{ padding: '8px 10px' }}>
                          {u.is_test && (
                            <span style={{ fontSize: 11, padding: '2px 6px', borderRadius: 4, background: '#fef9c3', color: '#854d0e', fontWeight: 600 }}>
                              teste
                            </span>
                          )}
                        </td>
...
function Metric({ label, value, sub, highlight }: { label: string; value: string; sub?: string; highlight?: boolean }) {
  return (
    <div style={{ background: '#fff', border: `1px solid ${highlight ? '#bfdbfe' : '#e2e8f0'}`, borderRadius: 8, padding: '12px 14px' }}>
      <div style={{ fontSize: 11, color: '#64748b', marginBottom: 4 }}>{label}</div>
      <div style={{ fontSize: 18, fontWeight: 700, color: highlight ? '#1d4ed8' : '#0f213f' }}>{value}</div>
