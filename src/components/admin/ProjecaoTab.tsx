...
interface BucketTotals { fal: number; openai: number; revenue: number; slots: number }
...
function calcEndDate(inicio: string | null, tipo: string): Date | null {
  ...
  if (tipo === 'mensal') { d.setMonth(d.getMonth() + 1); return d; }
  if (tipo === 'anual') { d.setFullYear(d.getFullYear() + 1); return d; }
  return null;
}
...
function buildProjections(profiles: Profile[], plans: Plan[], adminIds: Set<string>, s: Settings): SlotProj[] {
  ...
  for (const prof of profiles) {
    ...
    const slots = [
      { key: 'Plano 1', planId: prof.plano1_id, inicio: prof.plano1_inicio, iu: prof.plano1_imgs_usadas, il: prof.plano1_imgs_limite, ru: prof.plano1_renders_usados, rl: prof.plano1_renders_limite, gu: prof.plano1_geracoes_usadas, gl: prof.plano1_geracoes_limite, preco: (prof as any).plano1_preco_brl || 0 },
      { key: 'Plano 2', planId: prof.plano2_id, inicio: prof.plano2_inicio, iu: prof.plano2_imgs_usadas, il: prof.plano2_imgs_limite, ru: prof.plano2_renders_usados, rl: prof.plano2_renders_limite, gu: prof.plano2_geracoes_usadas, gl: prof.plano2_geracoes_limite, preco: (prof as any).plano2_preco_brl || 0 },
      { key: 'Bônus',   planId: prof.bonus_id,  inicio: prof.bonus_inicio,  iu: prof.bonus_imgs_usadas,  il: prof.bonus_imgs_limite,  ru: prof.bonus_renders_usados,  rl: prof.bonus_renders_limite,  gu: prof.bonus_geracoes_usadas,  gl: prof.bonus_geracoes_limite, preco: (prof as any).bonus_preco_brl  || 0 },
    ];
    for (const slot of slots) {
      ...
      out.push({
        ...
        remImgs, remRenders, remGeracoes, falCost, openaiCost, revenueBrl,
      });
    }
  }
  return out;
...
export function ProjecaoTab() {
  ...
  const [recharge, setRecharge] = useState<{ falai: string; openai: string }>({ falai: '', openai: '' });
  ...
  const load = useCallback(async () => {
    ...
    const [{ data: s }, { data: profs }, { data: pls }, { data: roles }] = await Promise.all([
      supabase.from('app_settings').select('usd_brl_rate,image_price_usd,render_price_usd,geracao_price_usd,falai_balance_usd,openai_balance_usd').eq('id', true).maybeSingle(),
    ...
    <div style={{ display: 'grid', gap: 24 }}>
      ...
      <section>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
          <SectionTitle style={{ margin: 0 }}>Gastos previstos — por vencimento de plano</SectionTitle>
          <button onClick={load} style={{ fontSize: 12, background: 'none', border: '1px solid #cbd5e1', padding: '4px 10px', borderRadius: 4, cursor: 'pointer', color: '#475569' }}>
            ↻ Atualizar
          </button>
        </div>
        {isMobile ? (
          <div style={{ display: 'grid', gap: 10 }}>
            {BUCKETS.map(b => {
              ...
              return (
                <div key={b} style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: 10, padding: 14 }}>
                  <div style={{ fontWeight: 700, fontSize: 13, marginBottom: 10 }}>{BUCKET_LABEL[b]}</div>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                    ...
                    <ProjCell label="Receita R$" value={t.revenue > 0 ? brl(t.revenue) : '—'} color="#15803d" />
                    <ProjCell label="Lucro R$" value={t.revenue > 0 ? brl(lucro) : '—'} color={lucro >= 0 ? '#15803d' : '#dc2626'} />
                    <ProjCell label="Planos" value={String(t.slots)} />
                  </div>
                </div>
              );
            })}
            <div style={{ background: '#f8fafc', border: '2px solid #e2e8f0', borderRadius: 10, padding: 14 }}>
              <div style={{ fontWeight: 800, fontSize: 13, color: '#0f213f', marginBottom: 10 }}>TOTAL PREVISTO</div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                <ProjCell label="fal.ai" value={usd(totalFal)} color="#6d28d9" />
                <ProjCell label="OpenAI" value={usd(totalOai)} color="#0369a1" />
                <ProjCell label="Custo R$" value={brl((totalFal + totalOai) * settings.usd_brl_rate)} />
                <ProjCell label="Receita R$" value={totalRev > 0 ? brl(totalRev) : '—'} color="#15803d" />
                <ProjCell label="Lucro R$" value={totalRev > 0 ? brl(totalRev - (totalFal + totalOai) * settings.usd_brl_rate) : '—'} color={(totalRev - (totalFal + totalOai) * settings.usd_brl_rate) >= 0 ? '#15803d' : '#dc2626'} />
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                ...
                {BUCKETS.map(b => {
                  ...
                  return (
                    <tr key={b} style={{ borderTop: '1px solid #e2e8f0' }}>
                      ...
                      <Td style={{ color: '#15803d', fontWeight: 600 }}>{t.revenue > 0 ? brl(t.revenue) : '—'}</Td>
                      <Td style={{ fontWeight: 700, color: lucro >= 0 ? '#15803d' : '#dc2626' }}>{t.revenue > 0 ? brl(lucro) : '—'}</Td>
              ...
              <tfoot style={{ background: '#f8fafc', borderTop: '2px solid #e2e8f0' }}>
                <tr>
                  ...
                  <Td style={{ fontWeight: 600, color: '#64748b' }}>{brl((totalFal + totalOai) * settings.usd_brl_rate)}</Td>
                  <Td style={{ fontWeight: 700, color: '#15803d' }}>{totalRev > 0 ? brl(totalRev) : '—'}</Td>
                  <Td style={{ fontWeight: 700, color: (totalRev - (totalFal + totalOai) * settings.usd_brl_rate) >= 0 ? '#15803d' : '#dc2626' }}>
                    {totalRev > 0 ? brl(totalRev - (totalFal + totalOai) * settings.usd_brl_rate) : '—'}
                  </Td>
      ...
      <section>
        ...
        {isMobile ? (
          <div style={{ display: 'grid', gap: 10 }}>
            {[
              ...
              const ok = tot === 0;
              return (
                <div key={label} style={{ background: ok ? '#f0fdf4' : '#fff', border: `1px solid ${ok ? '#bbf7d0' : '#e2e8f0'}`, borderRadius: 10, padding: 14 }}>
                  <div style={{ fontWeight: 700, fontSize: 13, marginBottom: ok ? 6 : 10, color: ok ? '#15803d' : '#0f213f' }}>{label}</div>
                  {ok ? (
                    <div style={{ color: '#15803d', fontWeight: 700, fontSize: 13 }}>✓ coberto pelo saldo atual</div>
                  ) : (
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                      <ProjCell label="fal.ai" value={fal === 0 ? '✓ coberto' : usd(fal)} color={fal === 0 ? '#15803d' : '#dc2626'} />
                      <ProjCell label="OpenAI" value={oai === 0 ? '✓ coberto' : usd(oai)} color={oai === 0 ? '#15803d' : '#dc2626'} />
                      <ProjCell label="Total USD" value={usd(tot)} color="#dc2626" />
                      <ProjCell label="Total R$" value={`R$ ${(tot * settings.usd_brl_rate).toFixed(2)}`} />
                    </div>
                  )}
                </div>
              );
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
              ...
              <tbody>
                {[
                  ...
                  const ok = tot === 0;
                  return (
                    <tr key={label} style={{ borderTop: '1px solid #e2e8f0', background: ok ? '#f0fdf4' : undefined }}>
                      <Td style={{ fontWeight: 600 }}>{label}</Td>
                      <Td style={{ color: fal === 0 ? '#15803d' : '#dc2626', fontWeight: 700 }}>{fal === 0 ? '✓ coberto' : usd(fal)}</Td>
                      <Td style={{ color: oai === 0 ? '#15803d' : '#dc2626', fontWeight: 700 }}>{oai === 0 ? '✓ coberto' : usd(oai)}</Td>
                      <Td style={{ fontWeight: 700, color: ok ? '#15803d' : '#dc2626' }}>{ok ? '✓ coberto' : usd(tot)}</Td>
                      <Td style={{ color: '#64748b' }}>{ok ? '—' : `R$ ${(tot * settings.usd_brl_rate).toFixed(2)}`}</Td>
                    </tr>
                  );
                })}
              </tbody>
        ...
        <div style={{ display: 'flex', gap: 10, marginTop: 10 }}>
          <SupplierLink href="https://fal.ai/dashboard/billing" color="#6d28d9" label="Recarregar fal.ai ↗" />
          <SupplierLink href="https://platform.openai.com/settings/organization/billing" color="#0369a1" label="Recarregar OpenAI ↗" />
        </div>
      </section>

      {/* ── Detalhe por usuário ── */}
      <section>
        <button onClick={() => setShowDetail(v => !v)} style={{ fontSize: 13, fontWeight: 600, background: 'none', border: '1px solid #cbd5e1', padding: '6px 14px', borderRadius: 6, cursor: 'pointer', color: '#0f213f', marginBottom: 10 }}>
        ...
        {showDetail && (
          <div style={{ border: '1px solid #e2e8f0', borderRadius: 8, overflow: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
              <thead style={{ background: '#f8fafc' }}>
                <tr>
                  <Th>Usuário</Th><Th>Slot</Th><Th>Plano</Th>
                  <Th>Vence em</Th><Th>Dias</Th>
                  <Th>Imgs rem.</Th><Th>Renders rem.</Th><Th>Ger. rem.</Th>
              ...
              <tbody>
                {[...projs].sort((a, b) => (a.daysLeft ?? 9999) - (b.daysLeft ?? 9999)).map((p, i) => {
                  const custoBrl = (p.falCost + p.openaiCost) * settings.usd_brl_rate;
                  const lucro = p.revenueBrl - custoBrl;
                  return (
                    <tr key={i} style={{ borderTop: '1px solid #f1f5f9', background: p.isTest ? '#fffbeb' : p.isAdmin ? '#f0f9ff' : undefined }}>
                      <Td>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                          <span style={{ fontWeight: 600 }}>{p.userName}</span>
                          {p.isTest && <span style={{ fontSize: 9, fontWeight: 700, background: '#d97706', color: '#fff', padding: '1px 4px', borderRadius: 3 }}>TEST</span>}
                          {p.isAdmin && <span style={{ fontSize: 9, fontWeight: 700, background: '#0369a1', color: '#fff', padding: '1px 4px', borderRadius: 3 }}>ADM</span>}
                        </div>
                      ...
                      <Td style={{ fontWeight: 700, color: p.daysLeft !== null && p.daysLeft <= 30 ? '#dc2626' : p.daysLeft !== null && p.daysLeft <= 60 ? '#d97706' : '#15803d' }}>
                      ...
                      <Td style={{ color: '#15803d', fontWeight: 600 }}>{p.revenueBrl > 0 ? brl(p.revenueBrl) : '—'}</Td>
                      <Td style={{ fontWeight: 700, color: p.revenueBrl > 0 ? (lucro >= 0 ? '#15803d' : '#dc2626') : '#94a3b8' }}>
...
function BalanceCard({ supplier, desc, balance, rate, color, rechargeVal, onRechargeChange, onRechargeConfirm, saving }: {
  supplier: string; desc: string; balance: number; rate: number; color: string;
  rechargeVal: string; onRechargeChange: (v: string) => void;
      ...
      <div style={{ borderTop: `1px solid ${color}20`, paddingTop: 10 }}>
        ...
        <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
          <input
            ...
            onKeyDown={(e) => { if (e.key === 'Enter') onRechargeConfirm(); }}
            style={{ flex: 1, padding: '6px 8px', border: `1px solid ${color}40`, borderRadius: 6, fontSize: 13, fontWeight: 600, color, background: '#fff', outline: 'none' }}
          />
          <button
...
function SupplierLink({ href, color, label }: { href: string; color: string; label: string }) {
  return (
    <a href={href} target="_blank" rel="noreferrer" style={{
      fontSize: 12, fontWeight: 700, color, border: `1px solid ${color}40`,
      padding: '6px 14px', borderRadius: 6, textDecoration: 'none', background: `${color}08`,
    }}>{label}</a>
  );
}

function SectionTitle({ children, style }: { children: React.ReactNode; style?: React.CSSProperties }) {
  return <h3 style={{ fontSize: 14, fontWeight: 700, color: '#0f213f', marginBottom: 10, ...style }}>{children}</h3>;
}

const Th = ({ children, style }: any) => (
  <th style={{ padding: '8px 10px', textAlign: 'left', fontSize: 12, color: '#475569', fontWeight: 600, whiteSpace: 'nowrap', ...style }}>{children}</th>
);
