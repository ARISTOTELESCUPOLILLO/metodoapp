...
export function DivulgacaoTab() {
  ...
  async function handleDownload(plan: Plan) {
    ...
    try { await generateCard(plan); } catch (e) { console.error('card error', e); }
    setDownloading(null);
  }
  ...
  return (
    <div style={{ display: 'grid', gap: 24 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 12 }}>
        <div>
          ...
          <p style={{ fontSize: 13, color: '#64748b', marginTop: 4, marginBottom: 0 }}>
            Cards 1080×1080 px para cada plano ativo.{' '}
            Defina o <strong>Preço máx</strong> em <em>Tabela de Preços</em> antes de baixar.
          </p>
        </div>
        <button
          ...
          return (
            <div key={plan.id} style={{ border: '1px solid #e2e8f0', borderRadius: 10, overflow: 'hidden' }}>
              ...
              <div style={{ position: 'relative', height: 76, overflow: 'hidden' }}>
                <img src={heroDesktopUrl} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover', objectPosition: 'center top', display: 'block' }} />
                <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: 32, background: 'linear-gradient(to bottom, rgba(15,33,63,0), #0f213f)' }} />
              </div>
              {/* Text preview */}
              ...
              {/* Footer */}
              <div style={{ padding: '10px 14px', background: '#f8fafc', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8 }}>
                <div>
                ...
                <button
                  ...
                  disabled={downloading !== null}
                  style={{
                    background: isDown ? '#64748b' : '#0f213f', color: '#fff',
      ...
      <p style={{ fontSize: 11, color: '#94a3b8', margin: 0 }}>
        Canvas 1080×1080 px · imagem da capa desktop no topo · fonte Inter.
        Defina os preços em <em>Tabela de Preços</em> e baixe novamente.
      </p>
    </div>
  );
}
