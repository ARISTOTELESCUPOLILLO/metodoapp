...
// ─────────────────────────────────────────────────────────────────────────
// VARIANTE "FECHO CARACTERÍSTICA" — teste A/B direcionado (09/07/2026),
// achado #1 da auditoria de conflitos do motor de Sugestão (Fable + crítica
// Sonnet, mesmo dia).
//
// Este arquivo é uma CÓPIA LITERAL de generateSugestao + judgeSugestaoEstrutural
// (src/core/sugestaoEngine.ts, estado atual de produção — já inclui a guarda
// "COERÊNCIA DO COMPLEMENTO" do commit 4d8f577) com UMA ÚNICA mudança
// cirúrgica: o critério 1 (fechoOk) do juiz estrutural.
//
// HIPÓTESE TESTADA: a guarda "COERÊNCIA DO COMPLEMENTO" (linha 779 de
// produção) já autoriza a IA a gerar um fecho modesto e verdadeiro
// (característica/uso concreto do item) quando não há benefício óbvio — mas
// o juiz `fechoOk` nunca foi atualizado e continua exigindo "resultado,
// necessidade ou benefício CONCRETO", fail-closed. Isso pode reprovar
// exatamente os fechos que a guarda nova autorizou, gerando retry
// desnecessário e possível degradação pra um benefício inflado.
//
// A ÚNICA VARIÁVEL desta cópia é o texto do critério fechoOk (marcado
// abaixo com "MUDANÇA"). Todo o resto — geração, D1, poda E2, loop de
// retry — é idêntico a produção.
//
// NÃO é importado por nenhuma rota de produção — existe só para o harness
// scripts/ab-sugestao/run.fechocaracteristica.harness.ts.
// ─────────────────────────────────────────────────────────────────────────
...
// Cópia de judgeSugestaoEstrutural (sugestaoEngine.ts:445-525) com a ÚNICA
// mudança desta variante: o critério 1 (fechoOk) — ver comentário "MUDANÇA"
// dentro do prompt. Retorna também `criterios` crus (não só ok/motivo) pra o
// harness conseguir logar qual critério especificamente reprovou em cada
// tentativa — diagnóstico que a função de produção não expõe.
export async function judgeSugestaoEstruturalFechoCaracteristica(
  apiKey: string,
  sugestao: string,
...
Responda JSON EXATAMENTE assim: { "fechoOk": true ou false, "nucleoOk": true ou false, "linguagemOk": true ou false, "especificoOk": true ou false, "economiaOk": true ou false, "contextoOk": true ou false, "motivo": "se algum item for false, 1 frase curta e objetiva dizendo o que corrigir; se todos forem true, string vazia" }`;
    ...
    const criterios = {
      fechoOk: parsed.fechoOk === true,
      nucleoOk: parsed.nucleoOk === true,
      linguagemOk: parsed.linguagemOk === true,
      especificoOk: parsed.especificoOk === true,
      economiaOk: parsed.economiaOk === true,
      contextoOk: parsed.contextoOk === true,
    };
    const allOk = Object.values(criterios).every(Boolean);
    if (allOk) return { ok: true, criterios };
    ...
    return {
      ...
      motivo:
        typeof parsed.motivo === "string" && parsed.motivo.trim()
          ? parsed.motivo.trim()
          : "juiz estrutural reprovou a frase sem detalhar o motivo",
    };
  } catch {
    return { ok: true };
  }
...
// Cópia de judgeSugestaoEstrutural (sugestaoEngine.ts:445-525) SEM NENHUMA
// mudança no critério fechoOk (texto idêntico ao de produção) — existe só
// pra dar ao BASELINE deste teste a mesma instrumentação (`criterios` cru)
// que a versão patched tem, permitindo comparar tentativas/reprovações de
// forma justa entre as duas (não só o texto final, como nos testes
// anteriores).
export async function judgeSugestaoEstruturalBaseline(
  apiKey: string,
  sugestao: string,
  concreteItem: string | null | undefined,
  mainActivity: string,
...
type JudgeFn = typeof judgeSugestaoEstruturalFechoCaracteristica;

// Núcleo compartilhado — cópia de generateSugestao (sugestaoEngine.ts,
// estado atual de produção, já com a guarda COERÊNCIA DO COMPLEMENTO) — a
// ÚNICA variável entre baseline e patched é QUAL FUNÇÃO DE JUIZ é chamada
// no loop de retry (parâmetro `judgeFn`), passada pelos 2 wrappers
// exportados no fim do arquivo. Todo o resto é idêntico a produção.
// Devolve diagnóstico de tentativas/reprovações pro harness comparar.
async function runGenerateSugestao(
  apiKey: string,
...
{ "sugestao": "1 frase, entre 4 e 7 palavras (máximo absoluto 7), em português, sem hashtag, sem emoji, sem aspas, concreta e de fácil compreensão" }`;
  ...
  let sugestao = "";
  let motivos: string[] = [];
  const motivosPorTentativa: string[][] = [];
  let reprovadoPorFechoOk = false;
  let tentativas = 0;
  ...
  for (let pass = 1; pass <= MAX_SUGGEST_ATTEMPTS; pass++) {
    ...
    if (motivos.length === 0) {
      ...
      if (!veredito.ok) {
        if (veredito.criterios && veredito.criterios.fechoOk === false) {
          reprovadoPorFechoOk = true;
        }
        motivos.push(
    ...
    motivosPorTentativa.push([...motivos]);
...
// BASELINE — juiz com fechoOk IDÊNTICO a produção (judgeSugestaoEstruturalBaseline).
export async function generateSugestaoFechoBaseline(
  apiKey: string,
  input: SugestaoEngineInput,
) {
  return runGenerateSugestao(apiKey, input, judgeSugestaoEstruturalBaseline);
}

// PATCHED — juiz com fechoOk ajustado (judgeSugestaoEstruturalFechoCaracteristica).
export async function generateSugestaoFechoCaracteristica(
  apiKey: string,
  input: SugestaoEngineInput,
) {
  return runGenerateSugestao(apiKey, input, judgeSugestaoEstruturalFechoCaracteristica);
}
